...
RELATIVE paths are preferable, as they are portable this means that, if
you share a relative path then the code won’t “break” Absolute paths are
...
To view the dataset in a new window, we can use the command view() BUT
this is best used by typing into the console, and not putting it in a
code chunk in the r markdown file. This is because including a command
that opens up a new window in an R markdown file can cause issues when
knitting

write\_csv(subdata, “./data/mlbsubtable.csv”) \<– this EXPORTS data\!
